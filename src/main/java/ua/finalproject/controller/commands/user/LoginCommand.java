...
public class LoginCommand implements Command {
     ...
     * Adds user to servlet context if he doesn't already logged into
     * the system.
     *
    ...
    public String execute(HttpServletRequest request) {
            ...
            request.setAttribute(RequestAttributes.INFORMATION_MESSAGE,
                    bundleManager
                            .getString(ValidationMessages
                                    .LOGIN_OR_PASSWORD_IS_MISSED));
            return JSPPages.INDEX_PAGE;
        }

        Optional<User> userOptional = userServiceImpl.findUserByLogin
                (login);
        if (!userOptional.isPresent() || !userOptional.get().getPassword
            ...
            logger.info(LogMessageBuilder.INSTANCE
                    .invalidAttemptOfLogInInfo(login));
            return JSPPages.INDEX_PAGE;
        }
        User user = userOptional.get();

        if (ContextUtil.checkUserAlreadyIsLogged(request.getSession(),
                login)) {
            request.setAttribute(RequestAttributes.INFORMATION_MESSAGE,
                    bundleManager
                            .getString(ExceptionMessages
                                    .USER_ALREADY_LOGGED));
            logger.info(LogMessageBuilder.INSTANCE.userAlreadyLoggedInfo
                    (login));
            return JSPPages.INDEX_PAGE;
        }

        request.getSession().setAttribute(RequestAttributes.USER_LOGIN,
                login);
        request.getSession().setAttribute(RequestAttributes.ROLE, user
                .getRole());
        logger.info(LogMessageBuilder.INSTANCE.userLogInInfo(login));
        request.setAttribute(RequestAttributes.INFORMATION_MESSAGE,
                bundleManager.getString(Messages.HELLO) +
                        GlobalConstants.WHITE_SPACE +
                        user.getFirstName() + GlobalConstants
                        .WHITE_SPACE + user
                        .getSecondName());
