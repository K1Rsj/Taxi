...
public enum QueryContainer {
    ...
    public String findForeignKeyIdFromTable(String tableName, String entityId, String idName){
        return "SELECT " + idName + " FROM " + tableName + " WHERE " + "id_" + tableName + " = " + entityId;
    }
    ...
    public String findOrdersByUserLogin(String login) {
        return "SELECT * FROM " + TableNames.USERS + " LEFT JOIN ORDERS on id_users = users_id where login =\"" + login + "\"";
    }
}
